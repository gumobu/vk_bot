...
class Messages:
    ...
    def __init__(self, peer_id=None, message=None, attachment=None, keyboard=None,
                 session=None, text=None, num=None,
                 event=None,
    ...
    def resend(self):
        ...
        if self.num == 1:  # Info chat
            return vk_session.method('messages.send', {'peer_id': <Chat1Id>, 'message': self.text,
                                                       'random_id': random.randint(-2147483648, +2147483648)})
        elif self.num == 2:  # Main chat
            return vk_session.method('messages.send', {'peer_id': <Chat2Id>, 'message': self.text,
                                                       'random_id': random.randint(-2147483648, +2147483648)})
    ...
    def response_determination(admin, response, event, keyboard):
        ...
        if admin is False:  # If not a private message from bot owner
            ...
            if response == '.команды':  # Sends a list of comands
                ...
                Messages(peer_id=event.message.peer_id,
                         message=f'Привет, {name}! Актуальное на '
                ...
                Messages(peer_id=event.message.peer_id,
                         message=f'Привет, {name}! Актуальный на {DateAndTime().now_cls.strftime("%d.%m.%Y")}'
                                 f' список группы',
                ...
                Messages(peer_id=event.message.peer_id, message=f'Ваши мемы прибыли, капитан {name}!',
                ...
                Messages(peer_id=event.message.peer_id,
                         message=f'Привет, {name}! Расписание дистанционных занятий,'
...
class DateAndTime:
    ...
    def __init__(self, element_id=None, start=False, time_param=None):
        ...
        self.get_hour_str = None
        self.add_minute_str = None
        self.add_hour_str = None
        self.delta_minute = None
        self.delta_hour = None
    ...
    def get_hour(self):
        ...
        if self.start is False:
            if datetime.strptime(data.end_time[int(self.id)], '%H:%M').replace(tzinfo=None).minute \
                ...
                self.delta_hour = datetime.strptime(data.end_time[int(self.id)], '%H:%M').replace(tzinfo=None).hour \
            ...
            if datetime.strptime(data.start_time[int(self.id)], '%H:%M').replace(tzinfo=None).minute \
                    - self.time_param.replace(tzinfo=None).minute < 0:
                self.delta_hour = int(datetime.strptime(data.start_time[int(self.id)],
                                                        '%H:%M').replace(tzinfo=None).hour) \
                                - self.time_param.replace(tzinfo=None).hour - 1
            else:
                self.delta_hour = datetime.strptime(data.start_time[int(self.id)], '%H:%M').replace(tzinfo=None).hour \
                                - self.time_param.replace(tzinfo=None).hour
        return int(self.delta_hour)
    ...
    def processing(self):
        ...
        if DateAndTime().week_counter() <= data.study_weeks_count:
            ...
            if day_id == (6 or 7):  # If it is saturday or sunday
                if odd is True:
                    ...
                    if self.week_counter() < 10:
                        return f'Сегодня {data.day_names[day_id]}, ' \
                               f'следующая пара в {data.day_names[1]}. \n' \
                               f'Это {data.classes[1]["list_of_classes"][0]}.\n' \
            ...
            if day_id == 4:  # If it is thursday
                for _ in range(0, len(data.classes[5]["num_of_classes"])):
                    start_true, end_true = DateAndTime(time_param=today, element_id=_ + 1).time_to()
                    if start_true or end_true is True:
                        break
                ...
                if start_true is True:
                    if self.week_counter() < 6:
                        return f'Следующая пара: {data.classes[5]["list_of_classes"][_]}.\n' \
                               f'До ее начала осталось' \
                               f' {Classes(time_param=today, element_id=_ + 1, start=True).word_total()}'
                    else:
                    ...
                    if _ < len(data.classes[5]["num_of_classes"]) - 1:
                        if self.week_counter() < 6:
                            return f'Текущая пара: {data.classes[5]["list_of_classes"][_]}.\n' \
                                   f'До ее конца осталось {Classes(time_param=today, element_id=_ + 1).word_total()}\n'\
            ...
            return 'Сегодня каникулы, ни о какой учебе не может быть и речи!'

def is_odd(param):
    """
    Finds out if the parameter odd or not
    """
    if param % 2 == 0:
        return True
    else:
        ...
        for event in events.listen():
            ...
            if event.type == VkBotEventType.MESSAGE_NEW:  # For messages in chat
                ...
                if int(event.message.peer_id) != 233071173:
                    response = algorithms.main(response, data.commands)
